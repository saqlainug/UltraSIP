...
/// (ULTRASIP_INTEGRATION=1 + ULTRASIP_PJSUA=<path>). Registration against a
...
final class SIPIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        guard let pjsuaPath = ProcessInfo.processInfo.environment["ULTRASIP_PJSUA"],
            FileManager.default.isExecutableFile(atPath: pjsuaPath)
        else {
            throw XCTSkip("ULTRASIP_PJSUA not set or not executable; run scripts/build-pjsip.sh first")
        }
    ...
    func testG729OnlyPeerCallWithMedia() async throws {
        ...
        // frames — same DTX behavior UltraSIP disables via noVad.
