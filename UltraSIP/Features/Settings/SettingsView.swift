...
struct GeneralSettingsView: View {
    ...
    var body: some View {
        Form {
            Toggle(
                "Launch UltraSIP at login",
                isOn: Binding(
                    get: { model.launchAtLogin },
                    set: { model.setLaunchAtLogin($0) }))
            if LaunchAtLogin.requiresApproval {
                Text("Approve UltraSIP in System Settings → General → Login Items to finish enabling this.")
                    .font(.caption2)
            ...
            LabeledContent("Window") {
                Text("Closing the window keeps UltraSIP running in the menu bar")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
