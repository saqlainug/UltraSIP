//
// USPEngine — the single owner of the PJSIP runtime.
//
...
//   converted to immutable USP* values; the Swift side republishes on the
//   main actor. Never call back into USPEngine synchronously from a
//   delegate callback.
...
extern NSErrorDomain const USPErrorDomain;

@protocol USPEngineDelegate <NSObject>
- (void)sipEngineRegistrationChanged:(USPRegistrationEvent *)event;
- (void)sipEngineCallChanged:(USPCallEvent *)event;
- (void)sipEngineIncomingCall:(USPCallEvent *)event;
@end

@interface USPEngine : NSObject

@property(nonatomic, weak, nullable) id<USPEngineDelegate> delegate;
...
/// call id later used in USPCallEvent.
