...
enum AppMainMenu {
    static func build(target: AppDelegate) -> NSMenu {
        ...
        appMenu.addItem(
            NSMenuItem(title: "Hide UltraSIP", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h"))
        appMenu.addItem(.separator())
        appMenu.addItem(
            NSMenuItem(title: "Quit UltraSIP", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
